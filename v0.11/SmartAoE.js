const SmartAoE = (() => {
    ...
    const buildDamageRow = function (damRoll1, damType1, hideformula, rowBackground, damRoll2="", damType2="", cardParameters) {
    ...
    const buildTableBody = function (saveRows, descRow, cardParameters) {
    ...
    const buildSaveRow = function (imageURL, id, name, success, saveInline, hideforumula, dam, autoApply, bar, markerString, rowBackground, cardParameters) {
        let result;
        let applyDamLink = autoApply ? '' : ` href='!smartapply ${id} ${bar} ${dam} ${markerString}'`;
    ...
    const makeInlineroll = (roll, hideformula, highlight=false) => {
          ...
          return "<span " +
            ...
            `style="display:inline-block;min-width:1em;font-size:1.2em;font-weight:bold;padding:0 3px;vertical-align:middle;${background}; ` +
    ...
    const makeAoELink = function(aoeType, aoeFloat, aoeColor, range, originType, originPt, minGridArea, minTokArea, originTokID, controlTokID, pathIDs, pageID, fxType, saveFormula, saveName, DC, damageBar, autoApply, damageFormula1, damageFormula2, damageBase1, damageBase2, damageType1, damageType2, rollDamage1, rollDamage2, conditionPass, conditionFail, cardParameters) {
    ...
    */
            ...
            for (let a=0; a<aoeLinks.indices.length; a++) {
                ...
                */
                ...
                if (aoeLinks.links[a].aoeType==='5econe') {
                    ...
                    path = await new Promise(function(resolve){
                        //let thePath = createPath(pathstring, pageID, 'gmlayer', 'transparent', '#ff0000', 3, rad*2, rad*2, originPtPx.x-z, originPtPx.y-z);
                        let thePath = createPath(pathstring, pageID, 'objects', 'transparent', '#ff0000', 3, rad*2, rad*2, originPtPx.x, originPtPx.y);
                        resolve(thePath);
                    });
                    //log('~~~~~~~~~~~~~~~~~~~~ SQUARE PATH~~~~~~~~~~~~~~~~~~~~');
                    //log(path)
    ...
    const applyDamage = function(tokID, bar, damage) {
        ...
        return `Applied ${damage} damage to ${name}`;
    }
    ...
    const processFinalMessages = function(messages, damageRolls, tokenRolls, aoeLink) {
        ...
        if (damageRolls.length > 0) {
            outputNew = outputNew + buildDamageRow(damageRolls[0].styled_1, aoeLink.damageType1, false, '#ffffff', '', '', aoeLink.cardParameters)
        }
        ...
        const saveRolls = tokenRolls.map((o, i) => {
            ...
            if (aoeLink.autoApply) {
                damageMessages.push(applyDamage(o.id, aoeLink.damageBar, thisDamage));
            }
            ...
            tableLineCounter += 1;
            if (tableLineCounter % 2 == 0) {
                rowColor = aoeLink.cardParameters.evenrowfontcolor
            } else {
                rowColor = aoeLink.cardParameters.oddrowfontcolor
            }
            return buildSaveRow(o.pic, o.id, o.name, success, o.styled_1, false, thisDamage, aoeLink.autoApply, aoeLink.damageBar, thisMarker, rowColor, aoeLink.cardParameters);
        });
    ...
    //const smartAoE_handleInput = function(msg) {
        ...
        try {
            ...
            if(msg.type=="api" && msg.content.toLowerCase().indexOf("!smartapply")==0) {
                ...
                let bar = args[2];
                let damage = args[3];
                let marker = args[4] || '';     //not all commands will include status marker(s)
                ...
                if (token) {
                    ...
                    sendChat(scriptName, `/w "${who}" TokenID ${tokenID} does not seem to exist. Perhaps it was deleted or the id is incorrect.`, null, {
                        noarchive: true
                    });
                }
            ...
            if(msg.type=="api" && msg.content.toLowerCase().match(/!smartping[gm|all]/)) {
                let whoDat = getObj('player',msg.playerid).get('_displayname');
                let args = msg.content.split(/\s+/);
                let tokenID = args[1];
                ...
                if (token) {
                    ...
                    sendChat(scriptName, `/w "${whoDat}" TokenID ${tokenID} does not exist. Perhaps it was deleted or the id is incorrect.`, null, {
                        noarchive: true
                    });
                }
            }
                ...
                spawnObj.get("_defaulttoken", async function(defaultToken) {
                    ...
                    */
                    ...
                    if (path) {
                        ...
                        let newLink = makeAoELink(aoeType, aoeFloat, aoeColor, range, originType, oPt, minGridArea, minTokArea, oTok.get('_id'), controlTok.get('_id'), path.get('_id'), controlTok.get('_pageid'), fxType, saveFormula, saveName, DC, damageBar, autoApply, damageFormula1, damageFormula2, damageBase1, damageBase2, damageType1, damageType2, rollDamage1, rollDamage2, conditionPass, conditionFail, cardParameters);
