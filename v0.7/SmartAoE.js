const SmartAoE = (() => {
    ...
    const makeAoELink = function(aoeType, range, originType, originPt, minArea, originTokID, controlTokID, pathIDs, pageID) {
    ...
    */
    ...
    const getConeLocations2 = function(aoeType, minArea, oPt, cPt, coneDirection, coneWidth, rad, pageGridIncrement, offsetX, offsetY) {
        let coneCoords = {
            xVals: [],
            yVals: []
        }
        ...
        //if (page) {
            ...
            switch (link.aoeType) {
                case '5econe':
                    ...
                    locationsArr = getConeLocations2(link.aoeType, link.minArea, originPtPx, controlPtPx, coneDirection, coneWidth, rad, pageGridIncrement, offsetX, offsetY)
                    break;
                case 'line':
        ...
        //delete the linked paths and clear the pathIDs array from the state object
            ...
            for (let a=0; a<aoeLinks.indices.length; a++) {
                ...
                */
                ...
                //Update the State object with new paths and originIndex
                updateAoELink(aoeLinks.indices[a], newPaths, aoeLinks.links[a].originIndex);
                //updateAoELink(aoeLinks.indices[a], aoeLinks.links[a].pathIDs);
            }
    }
    ...
    const smartAoE_handleInput = function(msg) {
        ...
        let minArea = 0;            //from 0 to 1, minimum fraction of grid cell to be "counted" as within AoE (ignored for line effects)
        let aoeLinks;
        ...
        try {
            ...
            */
            ...
            if(msg.type=="api" && msg.content.toLowerCase().indexOf("!rotateorigin")==0) {
                who = getObj('player',msg.playerid).get('_displayname');
                let cmd = msg.content.split(/\s+/);
                log(cmd);
                ...
                spawnObj.get("_defaulttoken", async function(defaultToken) {
                    ...
                    */
                    ...
                    if (path) {
                        ...
                        let newLink = makeAoELink(aoeType, range, originType, oPt, minArea, oTok.get('_id'), controlTok.get('_id'), path.get('_id'), controlTok.get('_pageid'));
