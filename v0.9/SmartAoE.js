const SmartAoE = (() => {
    ...
    const defaultParameters = {
		...
		title: "SmartAoE",                                     //done
		titlecardbackground: "linear-gradient(red, yellow)",    //done
		...
		subtitlefontcolor: "#333333",                           //done
		subtitleseperator: " &" + "#x2666; ",
		tooltip: "Sent by ScriptCards",
		bodyfontsize: "14px;", 
		bodyfontface: "Helvetica",
		oddrowbackground: "#F2E5BC",                    //done  light orange
		//oddrowbackground: "#FEF68E",                    //done  light yellow (same as inline roll)
		evenrowbackground: "#EEEEEE",                   //done  light grey
		oddrowfontcolor: "#ffffff",
		evenrowfontcolor: "#dddddd",
    ...
    const buildTitle = function (title, leftsub, rightsub, subtitleseperator) {
    //const buildTitle = (title, leftsub, rightsub, subtitleseperator) => {
        log('Entered BuildTitle');
        var subtitle = "";
		if ((leftsub !== "") && (rightsub !== "")) {
			subtitle = leftsub + subtitleseperator + rightsub;
		}
		if ((leftsub !== "") && (rightsub === "")) {
			subtitle = leftsub;
		}
    ...
    const buildDamageRow = function (damRoll1, damType1, hideformula, rowBackground, damRoll2="", damType2="") {
        ...
        if (damRoll2 !== "") {
            let damInline2 = makeInlineroll(damRoll2, hideformula);
        ...
        let output = `<table style="width:270px; border-collapse:collapse; table-layout:fixed; width 270px; font-size:!{bodyfontsize}; font-family: !{bodyfontface}; border-bottom: 2px solid #444444;">` +
                        `<tr style="background:${rowBackground}; text-align:center">${rowData}</tr>` +
                     `</table>`;
    ...
    const buildTableBody = function (saveRows, descRow) {
        ...
        let output = `<table style="width:270px; border-collapse:collapse; table-layout:fixed; font-size:!{bodyfontsize}; font-family: !{bodyfontface}">` +
                        `<tr style="background: #dddddd; text-align:center;border-bottom: 1px solid #444444;">` +
    ...
    const buildSaveRow = function (imageURL, name, success, saveInline, hideforumula, dam, rowBackground) {
        let result;
        log('entered buildSaveRow');
        //let saveInline = makeInlineroll(saveRoll, hideformula);
        
        if (success) {
            result = `<td style="text-align:center"><a style="font-weight:bold;color:#ffffff;background:#3FB315;border:1px solid black;padding:2px;cursor:pointer">Pass</a></td>`
        } else {
            result = `<td style="text-align:center"><a style="font-weight:bold;color:#ffffff;background:#b50e0c;border:1px solid black;padding:2px;cursor:pointer">Fail</a></td>`
        }
        let output = `<tr style="background: ${rowBackground}">` +
                        `<td style="padding:3px;height:25px">` +
                            `<div style="display:inline-block;height:25px;width:25px;vertical-align:middle;margin-right:4px;background:url('${imageURL}') 0/contain no-repeat"></div>` + 
                        ...
                        `</td>` +
                        `${result}` +
                        `<td style="text-align:center; font-weight:bold">${dam}</td>` +
                     `</tr>`;
    ...
    const buildDescRow = function (descText) {
        let output = `<desc style="background: #ffffff">` +
                        `<td colspan="4">` +
    ...
    const makeAoELink = function(aoeType, aoeColor, range, originType, originPt, minGridArea, minTokArea, originTokID, controlTokID, pathIDs, pageID, fxType, saveFormula, saveName, DC, damageFormula1, damageFormula2, damageBase1, damageBase2, damageType1, damageType2, rollDamage1, rollDamage2, desc) {
    ...
    */
    ...
    const processTokenRolls = function(token, formula, saveName, DC, damage, roll2x) {
        ...
        let computedFormula;
        if (character) {
            computedFormula = replaceAttributes(formula, charID);
        } else {
            return null;
        }
    ...
    const processMessagesMaster = function(messages, damageRolls, tokenRolls) {
        ...
        Promise.all(tokenRolls.map(o => new Promise((resolve) => {
            ...
            .then(async(messages) => processFinalMessages(messages, damageRolls, tokenRolls))
            .catch(sendErrorMessage);
    }
    
    
    //this was modified from Jakob's group-check api script
    const processFinalMessages = function(messages, damageRolls, tokenRolls) {
        //takes all the rolls that were processed thru the Roll20 die roller for all tokens and formats them.
        //   then, we will format the final chat output
        let freetext = "";
        let rowData = '';
        let saveName = tokenRolls[0].saveName;
        log('messages follows')
        log(messages)
        ...
        log(saveName)
        // Combine output
        const output = "/w GM " + makeBox(saveName, "subheader", freetext, rolls.join(""));
        //log(output)
        //sendChat('smartAoE', output);
        
        //log('BuildTitle')
        
        let outputNew = buildTitle('My Title','leftsub', 'rightsub',cardParameters.subtitleseperator);
        //let outputNew = buildTitle('My Title','leftsub', 'rightsub','~~');
        log(outputNew);
        
        let PF;
        log('num toks = ' + tokenRolls.length);
        
        const saveRolls = tokenRolls.map((o, i) => {
            let thisDamage = 0;
            log('name = ' + o.name)
            log('DC = ' + o.DC)
            log('result_1 = ' + o.result_1)
            if (o.result_1 >= o.DC) {
                success = true;
                thisDamage = Math.floor(damageRolls[0].result_1 / 2);
            } else {
                success = false;
                thisDamage = damageRolls[0].result_1;
            }
            log('calling buildSaveRow');
            
            let rowColor = 0;
            tableLineCounter += 1;
            if (tableLineCounter % 2 == 0) {
                rowColor = cardParameters.evenrowfontcolor
            } else {
                rowColor = cardParameters.oddrowfontcolor
            }
            return buildSaveRow(o.pic, o.name, success, o.styled_1, false, thisDamage, rowColor);
        });
        
        let allSaveRows = saveRolls.join("")
        log('allSaves');
        log(allSaveRows);
        let descRow = buildDescRow('Description here')
        log('adding TableBody');
        outputNew = outputNew + buildTableBody(allSaveRows, descRow) + '</table></div></div>';
        
        //buildTitle = function (title, leftsub, rightsub, subtitleseperator)
        //buildSaveRow = function (imageURL, name, success, saveInline, hideforumula, dam, rowBackground)
        //buildTableBody = function (saveRows, descRow)
        log(outputNew);
        sendChat('TEST','/w gm ' + outputNew);
        
        
        let command = 'hello'
        let output2 = `<div style="display: table; border: 2px solid #000000; background-color: #EEEEEE; width: 270px; text-align: left; border-radius: 6px; border-collapse: separate; box-shadow: 5px 3px 3px 0px #aaa;margin-left:-42px">` +
  `<div style="display: table-header-group; background-image: linear-gradient(red, yellow); border-bottom: 2px solid #444444;">` +
    `<div style="display: table-row;">` +
      `<div style="display: table-cell; padding: 2px 2px; text-align: center; border-bottom: 2px solid #444444;">` +
        `<span style="font-family: Contrail One; font-style:normal; font-size: 1.2em line-height: normal; font-weight: bold; color: #EEEEEE; text-shadow: -1px 1px 0 #000, 1px 1px 0 #000, 1px -1px 0 #000, -1px -1px 0 #000;">Thunderwave</span><br />` +
        `<span style="font-family: Tahoma; font-variant: normal; font-size: 13px; font-style:normal; font-weight: bold; color: #333333; ">CON Save DC 15 &#x2666 Slot level 3</span>` +
      `</div>` +
    `</div>` +
  `</div>` +
  `<table style="width:100%; border-collapse:collapse; table-layout:fixed; width 270px">` +
    `<tr style="background: #FFFFFF; text-align:center">` +
      `<td style="width:150px;padding:3px;height:10px;width:85%;vertical-align:middle;text-align:left;font-weight:bolder">Target</td>` +
      `<td style="width:30px;text-align:center;font-weight:bold;padding: 2px 2px">Roll</td>` +
      `<td style="width:40px;text-align:center; font-weight:bold; padding: 2px 2px">P/F?</td>` +
      `<td style="width:50px;font-weight:bold; padding: 2px 2px">Dam.</td>` +
    `</tr>` +
    `<tr style="background: #dddddd">` +
      `<td style="padding:3px;height:25px">` +
        `<div style="display:inline-block;height:30px;width:30px;vertical-align:middle;margin-right:4px;background:url('https://s3.amazonaws.com/files.d20.io/images/124219981/jHqH4p5DhRu8_bZPrPMODg/thumb.png?1587223613') 0/contain no-repeat"></div><span style="vertical-align:middle;font-weight:bolder">Yelric</span>` +
      `</td>` +
      `<td style="text-align:center">` +
        `<div class="showtip tipsy" title="Rolling d20 + (2&#42;(1-0)) &#91;PC&#93; + (2&#42;0) &#91;NPC&#93; = (14) + (2&ast;(1- 0))  + (2&ast;0) " style="min-width:1em;font-weight: bold;padding: .2em .2em;cursor:help;"><span style="background-color:rgba(254,246,142,1);padding:2px 2px">16</span></div>` +
      `</td>` +
      `<td style="text-align:center">` +
        `<a href="${htmlReplace(command)}" style="font-weight:bold;color:#ffffff;background:#3FB315;border:1px solid black;padding:2px;cursor:pointer">Pass</a>` +
      `</td>` +
      `<td style="text-align:center; font-weight:bold">9</td>` +
    `</tr>` +
    `<tr style="background: #ffffff">` +
      `<td style="padding:3px;height:25px">` +
        `<div style="display:inline-block;height:30px;width:30px;vertical-align:middle;margin-right:4px;background:url('https://s3.amazonaws.com/files.d20.io/images/26969873/PQI-LADko_oxnyeSVNN9kA/thumb.png') 0/contain no-repeat"></div><span style="vertical-align:middle;font-weight:bolder">Lich</span>` +
      `</td>` +
      `<td style="text-align:center">` +
        `<div class="showtip tipsy" title="Rolling d20 + (3&#42;(1-1)) &#91;PC&#93; + (3&#42;1) &#91;NPC&#93; = (1) + (3&ast;(1- 1))  + (3&ast;1) " style="min-width:1em;font-weight: bold;padding: .2em .2em;cursor:help;"><span style="background-color:rgba(254,246,142,1);padding:2px 2px;border:2px solid #b50e0c;">4</span></div>` +
      `</td>` +
      `<td style="text-align:center">` +
        `<a href="${htmlReplace(command)}" style="font-weight:bold;color:#ffffff;background:#b50e0c;border:1px solid black;padding:2px;cursor:pointer">Fail</a>` +
      `</td>` +
      `<td style="text-align:center; font-weight:bold">18</td>` +
    `</tr>` +
    `<desc style="background: #dddddd">` +
      `<td colspan="4">` +
      `<div style = "text-align:justify;text-justify:inter-word; margin:2px 2px 2px 2px">On a failed save, a creature takes damage and is **pushed 10 feet away from you**. On a successful save, the creature takes half as much damage and isn't pushed.In addition, unsecured objects that are completely within the area of effect are automatically pushed 10 feet away from you by the spell's effect, and the spell emits a thunderous boom audible out to 300 feet.</div>` +
      `</td>` +
    `</desc>` +
  `</table>` +
`</div>`
        //log(output2);
        //sendChat('TEST','/w gm ' + output2);
        ...
        for (let i=0; i<tokenRolls.length; i++) {
            rowData = buildRowOutput('', cardParameters, tokenRolls[i]);
    ...
    //const smartAoE_handleInput = function(msg) {
        ...
        let damageBase1 = 0;
        let damageBase2 = 0;
        let damageType1 = 'damageType1';
        let damageType2 = 'damageType2';
        let desc = ''
        let damageBar = 1;
        ...
        let validArgs = "range, aoetype, origin, minGridArea, minTokArea, fx, aoeColor";
            ...
            if(msg.type=="api" && msg.content.toLowerCase().indexOf("!smarttrigger")==0) {
                ...
                if (aoeLinks.indices.length > 0) {
                    ...
                    for (let a=0; a<aoeLinks.indices.length; a++) {
                        ...
                        //roll associated base damage
                        
                        log('rollDamage1 follows')
                        log(aoeLinks.links[a].rollDamage1);
                        if (aoeLinks.links[a].rollDamage1) {
                            ...
                            Promise.all(damageRolls.map(o => new Promise((resolve) => {
                                sendChat("", `${o.damageFormula1}`, resolve);
                            })))
                            .then((messages) => processDamageMessages(messages, damageRolls))
                            .catch(sendErrorMessage);
                            
                            log('damageRolls follows)')
                            log(damageRolls)
                            //let res = await rollDice(aoeLinks.links[a].damageFormula1);
                            //log('res follows')
                            //log(res);
                            */
                        }
                        ...
                        for (let t=0; t<thisValidToks.length; t++) {
                            ...
                            if (thisValidToks[t].overlapArea >= aoeLinks.links[a].minTokArea*thisValidToks[t].area) {
                                ...
                                if (foundIndex === -1) {
                                    tokenRolls.push(processTokenRolls(thisValidToks[t].tok, aoeLinks.links[a].saveFormula, aoeLinks.links[a].saveName, aoeLinks.links[a].DC, aoeLinks.links[a].damageBase1, roll2x));
                                }
                                
                                //sendChat(scriptName,`/w "${who}" `+ 'Token In area: ' + thisValidToks[t].tok.get("name") + '('+thisValidToks[t].overlapArea+') [[1d20]]');
                                
                                //log(aoeLinks.links[a].saveFormula + ', ' + aoeLinks.links[a].saveName);
                            }
                        }
                    }
                    ...
                    Promise.all(damageRolls.map(o => new Promise((resolve) => {
                        ...
                        .then(async(messages) => processMessagesMaster(messages, damageRolls, tokenRolls))
                        .catch(sendErrorMessage);
                    ...
                    Promise.all(tokenRolls.map(o => new Promise((resolve) => {
                        ...
                        .then(async(messages) => processFinalMessages(messages, tokenRolls))
                        .catch(sendErrorMessage);
                    */
                ...
                spawnObj.get("_defaulttoken", async function(defaultToken) {
                    ...
                    */
                    ...
                    if (path) {
                        ...
                        log(damageFormula1 + ',' + damageFormula2 + ',' + damageBase1 + ',' + damageBase2 + ',' + damageType1 + ',' + damageType2 + ',' + rollDamage1 + ',' + rollDamage2 + ',' + desc)
                        //log(damageFormula1)
                        //log(damageFormula2)
                        //log(damageBase1)
                        //log(damageBase2)
                        ...
                        let newLink = makeAoELink(aoeType, aoeColor, range, originType, oPt, minGridArea, minTokArea, oTok.get('_id'), controlTok.get('_id'), path.get('_id'), controlTok.get('_pageid'), fxType, saveFormula, saveName, DC, damageFormula1, damageFormula2, damageBase1, damageBase2, damageType1, damageType2, rollDamage1, rollDamage2, desc);
