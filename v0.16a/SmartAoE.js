    ...
    async function smartAoE_handleInput (msg) {
        ...
        try {
            ...
            if(msg.type=="api" && msg.content.toLowerCase().indexOf("!smartquery")==0) {
                ...
                let targetTok = getObj('graphic', msg.selected[0]._id);
                if (targetTok) {
                    let tokCenterPt = {x: targetTok.get("left"), y: targetTok.get("top")};
                    let aoeLinks = getAoELinks(msg.selected[0]._id, true);
                    ...
                    sendChat(scriptName, `${whisperString} You must select a token in order to proceed`, null, {
                        noarchive: true
                    });
                }
            }
