const SmartAoE = (() => {
    ...
    const getPathType = function() {
        const releaseType = Campaign().get("release");
    ...
    const updateAoELink = function(linkIndex, pathIDs, pathDimensions, originIndex, boundingBox) {
    //const updateAoELink = function(linkIndex, pathIDs, originIndex) {
        ...
        state[scriptName].links[linkIndex].pathDimensions = pathDimensions.map((x) => x);
    ...
    const build5eCone = function(rad, z, coneWidth, coneDirection) {
        ...
        */
        ...
        if (pathType == 'path') {
            ...
            pointsJSON = `[[${ptOrigin.x},${ptOrigin.y}],[${pt1.x},${pt1.y}],[${pt2.x},${pt2.y}],[${ptOrigin.x},${ptOrigin.y}]]`;
        }
    ...
    const buildWallPath = function(wallParams) {
        ...
        */
        ...
        if (pathType == 'path') {
            wallPointsJSON = `[[\"M\",${pt1.x},${pt1.y}],[\"L\",${pt2.x},${pt2.y}],[\"L\",${pt3.x},${pt3.y}],[\"L\",${pt4.x},${pt4.y}],[\"L\",${pt1.x},${pt1.y}]]`;
        } else {    //'pathv2'
            wallPointsJSON = `[[${pt1.x},${pt1.y}],[${pt2.x},${pt2.y}],[${pt3.x},${pt3.y}],[${pt4.x},${pt4.y}],[${pt1.x},${pt1.y}]]`;
        }
    ...
    const buildSquarePath = function(rad) {
        ...
        if (pathType == 'path') {
            ...
            squarePointsJSON = `[[${ptUL.x},${ptUL.y}],[${ptUR.x},${ptUR.y}],[${ptLR.x},${ptLR.y}],[${ptLL.x},${ptLL.y}],[${ptUL.x},${ptUL.y}]]`;
        }
    ...
    const buildCirclePath = function(rad, coneWidth, coneDirection) {
        ...
        if (coneWidth === 360) { 
            ...
            if (pathType == 'path') {
                ...
                circlePoints = JSON.stringify(acc)
                return circlePoints;
            }
        } else {
            ...
            //start path at the origin pt 
                ...
                for (let th=startAngle; th<endAngle+Number.EPSILON*steps; th+=stepSize) {
                    ...
                    x = oX + oX * Math.sin(th);
                    y = oY + oY * Math.cos(th + deg2rad*180);
                    circlePoints = circlePoints + `[${x},${y}],`
                }
                
                //connect back to the origin pt
                circlePoints = circlePoints + `[${oX},${oY}]`;
                //add "phantom" single points to path corresponding to the four corners to keep the size computations correct
                //circlePoints = circlePoints + `[${ptUL.x},${ptUL.y}],[${ptUL.x},${ptUL.y}],[${ptUR.x},${ptUR.y}],[${ptUR.x},${ptUR.y}],[${ptLR.x},${ptLR.y}],[${ptLR.x},${ptLR.y}],[${ptLL.x},${ptLL.y}],[${ptLL.x},${ptLL.y}],[${ptUL.x},${ptUL.y}],[${ptUL.x},${ptUL.y}]`;
                return circlePoints + "]";;
            }
        }
    ...
    */
    ...
    const getPathDimensions = function(pathstring){
        ...
        let pts = JSON.parse(pathstring).map(([x, y]) => ({x, y}))
        for (i=0; i < pts.length; i++) {
            //define parameters of bounding box
    ...
    const offsetConeOrigin = function(originPtPx, coneDirection, pathstring, z=0){
        let maxX = -Infinity;
        let maxY = -Infinity;
        let minX = Infinity;
        let minY = Infinity;
        
        //This is used to find the vertex of the cone within the given path (array of pts from pathstring)  
        coneDirectionInverse = normalizeTo360deg(coneDirection - 180);
        coneDirectionInverseRad = coneDirectionInverse * (Math.PI / 180);
        
        // Calculate the inverse direction vector components
        const inverseDirection = {
            x: Math.sin(coneDirectionInverseRad),   // Adjusted for the angle starting from the Y-axis
            y: -Math.cos(coneDirectionInverseRad)  
        }
        
        // Loop through each point and calculate the bouding box parameters and the projection of the pts in the inverse of the coneDirection vector. 
                //The farthest point from the inverse of the coneDirection vector will be the vertex
        let pts = JSON.parse(pathstring).map(([x, y]) => ({x, y}))
        let vertex = {x:null, y:null};
        ...
        for (i=0; i < pts.length; i++) {
            ...
            //finding the vertex
            let projection = (pts[i].x * inverseDirection.x + pts[i].y * inverseDirection.y);
            // Update the farthest point if the current projection is larger
        ...
        //log('vertex')
        //log(vertex)
        //find the center of bounding box of pts in absolute space
        let center = {  
            x: minX + (maxX - minX) / 2,
            y: minY + (maxY - minY) / 2
        };
        ...
        //the amount to offset will be the XY differences between the vertex and the center of the bounding box in absolute coordinates 
        let dX = center.x - (vertex.x);
        let dY = center.y - (vertex.y);
        //log('dx = ' + dX)
        //log('dy = ' + dY)
        return new pt(originPtPx.x + z + dX, originPtPx.y + z + dY);
    }
    ...
    async function updateMapWithAoE (aoeLinks, obj, recalcNearest=true, offsetX=0, offsetY=0) {
        ...
        //delete the linked paths and clear the pathIDs array from the state object
            ...
            for (let a=0; a<aoeLinks.indices.length; a++) {
                newPaths = [];
                pathDimensions = [];    //added to store path {w:a, h:b} because Jumpgate no longer stores these properties
                //log('a = ' + a);
                ...
                if (aoeLinks.links[a].outlineOnly===false) {
                    ...
                    for (let p=0; p<pathLocations.xVals.length; p++) {
                        ...
                        pathDimensions.push({w:70*pageGridIncrement, h:70*pageGridIncrement})
                    }
                }
                ...
                if (aoeLinks.links[a].aoeType==='5econe') {
                    ...
                    path = await new Promise(function(resolve){
                        if (pathType == 'pathv2') {
                            originPtPx = offsetConeOrigin(originPtPx, coneDirection, pathstring, z);
                        }
                    ...
                    path = await new Promise(function(resolve){
                        if (pathType == 'pathv2') {
                            originPtPx = offsetConeOrigin(originPtPx, coneDirection, pathstring, 0);
                        }
                ...
                if (aoeLinks.indices.length > 0) {
                    ...
                    for (let a=0; a<aoeLinks.indices.length; a++) {
                        ...
                        for (let p=0; p<aoeLinks.links[a].pathIDs.length; p++) {
                            let tempPath = getObj(pathType, aoeLinks.links[a].pathIDs[p])
                            if (tempPath) {
                ...
                args.forEach((arg) => {
                    ...
                    switch(option) {
                        ...
                        case "aoelayer":
                            ...
                            controlTokLayer = aoeLayer;
                            let pathType = getPathType();
                            if (pathType = 'pathv2') {
                                aoeLayer = 'foreground';    //override to foreground for Jumpgate
                            }
                            break;
