const SmartAoE = (() => {
    ...
    const buildSaveRow = function (imageURL, id, layer, name, success, saveInline1, saveInline2, roll2x, hideforumula, dam, autoApply, bar, markerString, RVI, zeroHPmarker, removeAtZero, cardParameters) {
        ...
        let applyDamLink = autoApply ? '' : ` href='!smartapply --args|${id}|${name}|${bar}|${damString}|${markerString}|${zeroHPmarker}|${removeAtZeroMsg}|${whisperDamageMsg}'`;
    ...
    const checkInstall = function() {
        ...
        /* --> The following is in progress, but getting a quick update out to fix a bug
        let controlTokenChars = findObjs({
            _type: 'character',
            name: 'AoEControlToken2'
        }, {caseInsensitive: true}) || [];
        if (controlTokenChars.length === 0) {
            log(`====> ${scriptName} controlTokenChar does not exist! Creating now.`)
            let charObj = {
                //avatar: 
            }
        }
    ...
    async function smartAoE_handleInput (msg) {
        ...
        try {
            ...
            if(msg.type=="api" && msg.content.toLowerCase().indexOf("!smartapply")==0) {
                ...
                //let args = msg.content.split(/\s+/);
                let args = msg.content.split(/\s+--args/);
                if (args.length > 1) {
                    args = args[1].split(/\|/);
                }
            ...
            if(msg.type=="api" && msg.content.toLowerCase().indexOf("!smartaoe")==0) {
                ...
                if (radius !== 'variable' && (aoeFloat !== true || controlTokName.toLowerCase()=== 'self') && (aoeType === 'square' || aoeType === 'circle' || aoeType === 'line')) {
