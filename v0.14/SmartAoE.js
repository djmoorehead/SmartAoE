    ...
    */
    ...
    const getConeLocations2 = function(pageGridCenters, aoeType, minGridArea, oPt, cPt, coneDirection, coneWidth, rad, pageGridIncrement, offsetX, offsetY) {
        ...
        if (aoeType === '5econe') {
            ...
            coneEndPts = getConeEndPts2(aoeType, oPt, coneDirection, coneWidth, rad, z);    //3-element array with the origin and endPts of the cone (origin pt is excluded from this array)
        ...
        //if (page) {
            ...
            switch (link.aoeType) {
                case '5econe':
                    ...
                    locationsArr = getConeLocations2(pageGridCenters, link.aoeType, link.minGridArea, originPtPx, controlPtPx, coneDirection, link.coneWidth, rad, pageGridIncrement, offsetX, offsetY)
                    break;
                case 'cone':
                    coneDirection = getAngle2ControlToken(originPtPx, controlPtPx);
                    if (link.radius === 'variable') {
                ...
                */
                ...
                for (let p=0; p<pathLocations.xVals.length; p++) {
                    ...
                    path = await new Promise(function(resolve){
                        ...
                        let thePath = createPath(pathstring, pageID, 'objects', aoeLinks.links[a].aoeColor, '#000000', 2, 70*pageGridIncrement, 70*pageGridIncrement, pathLocations.xVals[p], pathLocations.yVals[p]);
                        resolve(thePath);
                    });
    ...
    //const smartAoE_handleInput = function(msg) {
        ...
        try {
            ...
            if(msg.type=="api" && msg.content.toLowerCase().indexOf("!smartapply")==0) {
                ...
                if (token) {
                    damageMsg = applyDamage(tokenID, name, bar, damageArr, zeroHPmarker);
            ...
            if(msg.type=="api" && msg.content.toLowerCase().indexOf("!smarttrigger")==0) {
               let player = getObj('player',msg.playerid);
                ...
                //    t.get("name").toLowerCase() !== 'aoecontroltoken' & 
            ...
            if(msg.type=="api" && msg.content.toLowerCase().indexOf("!smartaoe")==0) {
                ...
                spawnObj.get("_defaulttoken", async function(defaultToken) {
                    ...
                    let path = createPath(pathstring, pageID, 'objects', aoeColor, '#000000', 2, 70*pageGridIncrement, 70*pageGridIncrement, controlTok.get("left"), controlTok.get("top"));
                    ...
                    if (path) {
                        ...
                        let newLink = makeAoELink(controlTokName, aoeType, coneWidth, aoeFloat, forceIntersection, aoeColor, aoeOutlineColor, radius, originType, oPt, minGridArea, minTokArea, oTok.get('_id'), controlTok.get('_id'), path.get('_id'), controlTok.get('_pageid'), fxType, saveFormula, saveName, ignoreAttr, ignoreVal, DC, damageBar, autoApply, damageFormula1, damageFormula2, damageBase1, damageBase2, damageType1, damageType2, rollDamage1, rollDamage2, damageSaveRule, resistanceRule, vulnerableRule, immunityRule, resistAttr, vulnerableAttr, immunityAttr, conditionPass, conditionFail, zeroHPmarker, hideNames, cardParameters);
